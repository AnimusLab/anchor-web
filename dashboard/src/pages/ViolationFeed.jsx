...
const MOCK_VIOLATIONS = [
  { id: 'v_9921', project: 'Retail Customer Chatbot', severity: 'CRITICAL', time: '12 mins ago', title: 'Unapproved PII Access Pattern detected in Chat Handler.', status: 'OPEN' },
  { id: 'v_9918', project: 'Credit Scoring Engine v4', severity: 'WARNING', time: '2 hours ago', title: 'Model drift threshold exceeded on bias metric.', status: 'ACKNOWLEDGED' },
  { id: 'v_9850', project: 'Fraud Detection Mesh', severity: 'CRITICAL', time: '1 day ago', title: 'Cryptographic signature mismatch on node sync.', status: 'RESOLVED' },
];
            ...
            {violations.map(v => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--text-dim)' }}>{v.time}</span>
                </div>
                <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 6 }}>{v.title}</div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{v.project}</span>
