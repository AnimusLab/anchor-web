...
export default function AdminLoginPage() {
  ...
  return (
    <div className="min-h-screen relative flex flex-col justify-between p-6 md:p-10 bg-slate-50 dark:bg-[#09090C] text-slate-900 dark:text-slate-100 overflow-hidden font-sans selection:bg-rose-500/40 selection:text-rose-100 transition-colors duration-300">
      {/* Ambient Smooth Merged Glow Orbs */}
      <div className="ambient-glow-orb -top-32 -left-32 w-[600px] h-[600px] bg-gradient-to-br from-rose-500/20 via-pink-600/15 to-red-500/20 dark:from-rose-500/40 dark:via-pink-600/30 dark:to-red-500/40 animate-spatial-aurora" />
      <div className="ambient-glow-orb -bottom-32 -right-32 w-[600px] h-[600px] bg-gradient-to-br from-pink-600/20 via-rose-600/15 to-purple-600/20 dark:from-pink-600/40 dark:via-rose-600/30 dark:to-purple-600/40 animate-spatial-aurora" style={{ animationDelay: "-6s" }} />
      ...
      <header className="flex items-center justify-between z-20 max-w-7xl w-full mx-auto pb-6 border-b border-slate-200 dark:border-white/20">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <span className="text-[11px] font-mono text-rose-600 dark:text-rose-300 font-bold tracking-widest block uppercase">
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-3 text-xs font-mono">
            <span className="w-2.5 h-2.5 rounded-full bg-rose-500 dark:bg-rose-400 animate-ping" />
      ...
      <main className="z-20 max-w-7xl w-full mx-auto my-auto py-8 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center justify-items-center">
        ...
        <div className="lg:col-span-6 w-full max-w-lg pure-glass-card p-8 md:p-10 rounded-3xl space-y-6">
          <div>
            <div className="inline-flex items-center space-x-2 bg-rose-500/15 dark:bg-rose-500/20 backdrop-blur-md border border-rose-400/40 px-3.5 py-1.5 rounded-full text-xs font-mono text-rose-700 dark:text-rose-200 mb-3 shadow-inner">
              <Layers className="w-4 h-4 text-rose-600 dark:text-rose-300" />
            ...
            <p className="text-sm text-slate-600 dark:text-slate-200 mt-2 leading-relaxed font-sans">
              {step === "totp"
                ...
                : "Enter your Access Credential ID to authenticate platform admin access."}
            </p>
          </div>
              ...
              <>
                <div>
                  <div className="flex items-center justify-between mb-2 font-mono">
                    <label className="block text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider">
                      Access Credential ID <span className="text-rose-600 dark:text-rose-300">*</span>
                    </label>
                    {isScanning && (
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider mb-2 font-mono">
                    Corporate Email <span className="text-rose-600 dark:text-rose-300">*</span>
                  </label>
                  <input
                    type="email"
                    required
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider mb-2 font-mono">
                    Organization Hub ID <span className="text-rose-600 dark:text-rose-300">*</span>
                  </label>
                  <input
                    type="text"
                    required
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full bg-rose-600 hover:bg-rose-500 dark:bg-gradient-to-r dark:from-rose-600 dark:via-rose-500 dark:to-pink-600 hover:dark:from-rose-500 hover:dark:to-pink-500 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-black py-4 rounded-2xl shadow-lg hover:shadow-xl transition-all uppercase tracking-wider flex items-center justify-center space-x-2 border border-rose-400/30"
                >
                  <span>{isLoading ? "AUTHENTICATING..." : "AUTHENTICATE ROOT SESSION →"}</span>
                </button>
              </>
            ) : (
              <>
                {/* STEP 2: 2FA TOTP Code Entry */}
                <div className="p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-500/30 space-y-2.5 font-mono text-xs shadow-sm">
                  <div className="flex items-center justify-between text-[11px] font-bold text-rose-700 dark:text-rose-300">
                    <span>CLEARANCE ID: <strong className="text-slate-900 dark:text-white font-mono">{clearanceId || "AN-ADMIN-ROOT"}</strong></span>
                    <span>HUB: <strong className="text-rose-700 dark:text-rose-300">{hubId || "animuslab-hq"}</strong></span>
                  </div>
                  <div className="text-[11px] text-slate-700 dark:text-slate-300">
                    USER: <span className="font-semibold text-slate-900 dark:text-white">{email}</span>
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider mb-2 font-mono">
                    6-Digit Authenticator Code (TOTP) <span className="text-rose-600 dark:text-rose-300">*</span>
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    placeholder="000000"
                    autoFocus
                    className="w-full pure-glass-input rounded-2xl px-6 py-4 text-center tracking-[0.5em] text-2xl font-mono text-slate-900 dark:text-white focus:outline-none transition"
                  />
                </div>
          ...
          <div className="pt-3 border-t border-slate-200 dark:border-white/20 space-y-2.5">
            ...
            <button
              ...
              className="w-full bg-slate-100 hover:bg-slate-200 dark:bg-sky-500/10 dark:hover:bg-sky-500/20 border border-slate-300 dark:border-sky-400/30 text-slate-700 dark:text-sky-200 py-2.5 rounded-2xl font-mono text-[11px] font-bold tracking-wider uppercase transition flex items-center justify-center space-x-2 cursor-pointer"
            >
      ...
      <footer className="z-20 max-w-7xl w-full mx-auto pt-6 border-t border-slate-200 dark:border-white/20 flex items-center justify-between text-xs font-mono text-slate-500 dark:text-slate-300">
        ...
        <div className="text-slate-800 dark:text-slate-200 font-bold tracking-wider">
          SOVEREIGN RELAY ACTIVE · <span className="text-rose-600 dark:text-rose-300 font-mono">ANIMUSLAB.DEV</span>
        </div>
      </footer>
    </div>
  );
