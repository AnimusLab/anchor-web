...
export default function LoginPage() {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.message || "Failed to authenticate node credential.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#03050a] text-slate-100 flex flex-col justify-between p-6 md:p-10 relative overflow-hidden font-sans selection:bg-indigo-500/40 selection:text-indigo-100">
      ...
      <div className="absolute -top-32 -left-32 w-[550px] h-[550px] rounded-full bg-gradient-to-r from-indigo-500/40 via-purple-600/40 to-cyan-500/40 blur-[140px] pointer-events-none animate-spatial-aurora" />
      <div className="absolute -bottom-32 -right-32 w-[550px] h-[550px] rounded-full bg-gradient-to-r from-blue-600/40 via-indigo-600/40 to-fuchsia-500/40 blur-[140px] pointer-events-none animate-spatial-aurora" style={{ animationDelay: "-6s" }} />
      ...
      <main className="z-20 max-w-[1550px] w-full mx-auto my-auto py-8 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        {/* Left Form Container (Shifted Left, Enlarged Inputs) */}
        <div className="lg:col-span-5 w-full max-w-xl justify-self-start pure-glass-card p-8 md:p-11 rounded-3xl space-y-6">
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-indigo-600 via-indigo-500 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white text-base font-black py-4.5 rounded-2xl shadow-[0_0_35px_rgba(99,102,241,0.6)] hover:shadow-[0_0_50px_rgba(99,102,241,0.8)] transition-all uppercase tracking-wider flex items-center justify-center space-x-2 border border-indigo-300/40"
            >
        ...
        {/* Right Enlarged Horizontal ID Card Container (Shifted Right) */}
